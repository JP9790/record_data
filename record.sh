...
rosbag record --quiet -O ${filename}.bag -e "(.*)/iiwa/PositionController/command" \
    "(.*)/iiwa/joint_states" "(.*)/iiwa/TorqueController/command" "(.*)/record_data" &
...
poseinfo=`echo $info | grep -e '\ \/[^ ]*' -o |grep -e '/record_data'`
...
rostopic echo -p -b ${filename}.bag $poseinfo > ${filename}_pose.csv 
